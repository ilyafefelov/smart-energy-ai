...
class TestPerformanceLoad:
    ...
    async def test_optimization_performance(self):
        ...
        try:
            ...
            request = {"user_preference": "balance", "hours_ahead": 24}
            ...
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    assert execution_time < 30.0, (
                        f"Optimization too slow: {execution_time}s"
                    )
    ...
    def test_battery_calculation_speed(self):
        ...
        try:
            ...
            for i in range(iterations):
                ...
                degradation = battery.calculate_degradation(
                    power_kw=2.5, duration_hours=0.1
                )
            ...
            assert total_time < 1.0, (
                f"Battery calculations too slow: {total_time}s for {iterations} iterations"
            )
            assert time_per_calculation < 0.001, (
                f"Individual calculation too slow: {time_per_calculation}s"
            )
    ...
    def test_control_system_response_time(self):
        ...
        try:
            ...
            assert execution_time < 0.1, (
                f"Command execution too slow: {execution_time}s"
            )
...
class TestMemoryUsage:
    ...
    def test_battery_model_memory(self):
        ...
        try:
            ...
            assert peak_mb < 50, (
                f"Memory usage too high: {peak_mb}MB for 100 battery instances"
            )
    ...
    def test_continuous_operation_simulation(self):
        ...
        try:
            ...
            for minute in range(1440):  # 24 * 60 minutes
                ...
                battery.update_soc(power_kw=power, duration_hours=1 / 60)

                # Update controller
                controller.current_power_kw = power
                controller.current_soc = battery.state.soc
            ...
            assert simulation_time < 10.0, (
                f"Simulation too slow: {simulation_time}s for 24h simulation"
            )
        ...
        try:
            ...
            def run_battery_simulation(battery_id):
                try:
                    ...
                    for i in range(100):
                        degradation = battery.calculate_degradation(
                            power_kw=2.0, duration_hours=0.1
                        )
                        efficiency = battery.get_efficiency(power_kw=2.0, soc=0.5)
                        battery.update_soc(power_kw=2.0, duration_hours=0.1)  # 0.1 hour

                    results.put({"battery_id": battery_id, "success": True})

                except Exception as e:
                    results.put(
                        {"battery_id": battery_id, "success": False, "error": str(e)}
                    )
            ...
            assert successful >= 4, (
                f"Too many concurrent battery simulations failed: {successful}/5"
            )
