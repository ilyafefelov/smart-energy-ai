...
class TestPriceProcessor:
    ...
    def test_moving_average_smoothing(self, sample_prices):
        ...
        print(f"✓ Smoothing reduces volatility: {original_std:.2f} → {smoothed_std:.2f}")
    ...
    def test_zscore_standardization(self, sample_prices):
        ...
        print(f"✓ Z-score standardization: mean={standardized.mean():.6f}, std={standardized.std():.6f}")
    ...
    def test_denormalization(self, sample_prices):
        ...
        normalized = processor.normalize_prices()
        denormalized = processor.denormalize_prices(normalized.values)
        
        # Should match original (within floating point tolerance)
        np.testing.assert_array_almost_equal(denormalized, sample_prices.values, decimal=5)
    ...
    def test_full_pipeline(self, sample_prices):
        ...
        print(f"✓ Full pipeline: {len(processed)} hours processed with all transformations")
    ...
    def test_price_statistics(self, sample_prices):
        ...
        print(f"✓ Price statistics: min={stats['min_price_uah']}, max={stats['max_price_uah']}, avg={stats['mean_price_uah']:.2f}")
