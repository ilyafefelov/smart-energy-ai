...
class TestVirtualInverterController:
    def setup_method(self):
        self.controller = VirtualInverterController(battery_capacity_kwh=10.0, max_power_kw=5.0)
    ...
    async def test_soc_limits(self):
        ...
        try:
            ...
            # Should either reject or reduce power
            result = await self.controller.execute_command(action)
            assert result['success'] is True
            # Power should be reduced or zero due to SOC limits
            assert self.controller.current_power_kw <= 3.0
        except AttributeError as e:
