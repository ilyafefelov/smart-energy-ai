...
def test_active_docs_point_to_canonical_runtime_surfaces() -> None:
    ...
    assert "scripts/ml_integration_api.py — canonical Python bridge" in stage2_plan
    ...
    assert "archive/streamlit_dashboard_legacy_20260413/" in configuration_system
    assert "dashboard/ is the canonical operator UI" in configuration_system

    assert "├── streamlit_dashboard/" not in project_schematic
    assert "streamlit_dashboard_legacy_20260413/" in project_schematic
