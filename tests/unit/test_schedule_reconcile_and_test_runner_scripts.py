...
def test_run_tests_command_helpers_and_main(monkeypatch, capsys) -> None:
    module = load_script_module("scripts.run_tests_under_test", "scripts/run_tests.py")

    monkeypatch.setattr(module.subprocess, "run", lambda *args, **kwargs: SimpleNamespace(returncode=0, stdout="ok", stderr=""))
    assert module.run_command([sys.executable, "-V"], "Version Check") is True
    success_output = capsys.readouterr().out
    assert "SUCCESS" in success_output

    def timeout_run(*args, **kwargs):
        raise module.subprocess.TimeoutExpired(cmd="pytest", timeout=30)

    monkeypatch.setattr(module.subprocess, "run", timeout_run)
    assert module.run_command([sys.executable, "-m", "pytest"], "Timeout Check") is False
    timeout_output = capsys.readouterr().out
    assert "TIMEOUT" in timeout_output

    monkeypatch.setattr(module, "check_test_environment", lambda: True)
    monkeypatch.setattr(module, "run_unit_tests", lambda: True)
    monkeypatch.setattr(module, "run_integration_tests", lambda: False)
    monkeypatch.setattr(module, "run_e2e_tests", lambda: True)
    monkeypatch.setattr(module, "run_performance_tests", lambda: True)
    ...
    assert exit_code == 1
    assert "UNIT" in summary_output
    assert "INTEGRATION" in summary_output
    assert "1 test suite(s) failed" in summary_output


def test_run_tests_env_check_and_coverage_dispatch(monkeypatch, capsys) -> None:
    module = load_script_module("scripts.run_tests_coverage_under_test", "scripts/run_tests.py")
    existing = {
        "tests": True,
        "energy_ml": True,
        "tests/unit/test_battery_physics.py": True,
        "tests/unit/test_control_system.py": False,
        "tests/integration/test_api_integration.py": False,
        "tests/e2e/test_dashboard_e2e.py": False,
    }
    monkeypatch.setattr(module.os.path, "exists", lambda path: existing.get(path, False))
    assert module.check_test_environment() is True
    env_output = capsys.readouterr().out
    assert "directory found" in env_output
    assert "Some test files missing" in env_output

    monkeypatch.setattr(module, "check_test_environment", lambda: True)
    monkeypatch.setattr(module, "run_with_coverage", lambda: True)
    monkeypatch.setattr(sys, "argv", ["run_tests.py", "--coverage"])
    exit_code = module.main()
    coverage_output = capsys.readouterr().out

    assert exit_code == 0
    assert "ALL TESTS PASSED" in coverage_output