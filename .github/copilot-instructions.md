...
- `energy_ml/pipeline.py` live-context price parsing, battery parsing, and fallback behavior are covered by `tests/unit/test_optimizer_and_pipeline_surfaces.py`; preserve invalid live price row skipping plus alias support for `soc`/`health` battery keys when thinning that module.
