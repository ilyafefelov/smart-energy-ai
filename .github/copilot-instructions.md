...
- `energy_ml/pipeline.py` recommendation detail assembly and `price_source` selection are also covered by `tests/unit/test_optimizer_and_pipeline_surfaces.py`; preserve the current `details` payload shape when extracting reporting helpers there.
