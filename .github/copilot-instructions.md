...
- `src/data_pipeline/ml_bridge_service.py` is the canonical backend Python ML bridge owner; keep `scripts/ml_integration_api.py` as the thin CLI and compatibility adapter.
...
- Benchmark-backed forecast promotion metadata and model readiness now live in `src/data_pipeline/forecast_model_registry.py`; keep `src/assets/core/price_forecast.py` consuming only validated promotion metadata from that registry and do not reintroduce env-toggle or ad hoc runtime selection paths.
