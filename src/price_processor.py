...
class PriceProcessor:
    ...
    def __init__(self, prices_uah: pd.Series, normalize: bool = True, add_noise: bool = False):
        ...
        """
        self.original_prices = prices_uah.copy()  # Keep original for reference
        self.prices_uah = prices_uah.copy()
    ...
    def add_moving_average_smoothing(self, window: int = 3) -> pd.Series:
        """
        ...
        Returns:
            Smoothed prices
        """
        smoothed = self.prices_uah.rolling(window=window, center=True, min_periods=1).mean()
    ...
    def add_realistic_noise(self, noise_level: float = 0.05) -> pd.Series:
        """
    ...
    def normalize_prices(self, prices: pd.Series = None) -> pd.Series:
        """
    ...
    def standardize_prices(self, prices: pd.Series = None) -> pd.Series:
        """
    ...
    def denormalize_prices(self, normalized_prices: np.ndarray) -> np.ndarray:
        """
        ...
        Returns:
            Prices in UAH/MWh
        """
        denormalized = normalized_prices * (self.price_max - self.price_min) + self.price_min
        return denormalized
    ...
    def process_for_training(self) -> pd.DataFrame:
        """
        ...
        if self.normalize:
            ...
            result['price_normalized_minmax'] = normalized  # 0-1 range
            result['price_standardized_zscore'] = standardized  # mean=0, std=1
...
def prepare_prices_for_rl(df: pd.DataFrame, normalize: bool = True, 
                          add_noise: bool = False) -> Tuple[pd.DataFrame, dict]:
    """
